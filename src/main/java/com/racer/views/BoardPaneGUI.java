...
import com.racer.listeners.StartQuestionListener;
...
public class BoardPaneGUI extends JFrame {
	...
	public BoardPaneGUI(RacerBoard rb) {
		...
		btnRollDice = new RacerButton("Tirar los dados", RacerPanel.getSecondaryColor(), RacerPanel.getPrimaryColor(),
		...
		textPaneAction.setText("Acci�n:");
		textPaneAction.setBackground(RacerPanel.getSecondaryColor().brighter());
		scrollPaneAction.setViewportView(textPaneAction);

		iconDice = new RacerIcon(new ImageIcon(FileSystems.getDefault().getPath("img/dice", "dice1.png").toString()),
				150);
		...
		iconActionDice = new RacerIcon(
				new ImageIcon(FileSystems.getDefault().getPath("img/actionDice", "actionRed.png").toString()), 150);
		...
		panelBoardGrid = new GridBoard(FileSystems.getDefault().getPath("img", "racerCircuitBoard.png").toString(),
				450);
		...
		btnRollDice.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				iconActionDice.setIcon(
						new ImageIcon(FileSystems.getDefault().getPath("img/actionDice", "actionDice.gif").toString()),
						150);
				iconDice.setIcon(
						new ImageIcon(FileSystems.getDefault().getPath("img/dice", "rollingDice.gif").toString()), 150);
				...
				Timer rollingAnimation = new Timer(1000, new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						...
						textPaneAction.setText("Acci�n: " + rb.getCurrentActionDesc());
						dicesPanel.updateUI();
						iconDice.setIcon(
								new ImageIcon(FileSystems.getDefault()
										.getPath("img/dice", "dice" + rb.getDice().getValue()).toString() + ".png"),
								150);
		...
		btnStartQuestion.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				questionPanel = new QuestionPanel(rb, panelBoardGrid, btnStartQuestion, btnEndTurn, textPaneAction,
						rb.getPlayerToAnswerPanelCard());

				startQuestionListener.listenStartQuestion(
						new StartQuestionEvent(rb.getPlayerToAnswer(), rb.getCurQuestion(), questionPanel));
		...
		dicesPanel = new RacerPanel(new MigLayout("fill", "[grow]", "[][grow][grow][grow][]"),
				RacerPanel.getPrimaryColor(), RacerPanel.getSecondaryColor().darker(), RacerPanel.getSecondaryColor());
		...
		btnExit.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				if (JOptionPane.showConfirmDialog(null, "�Seguro que quieres salir de la partida?", "�Cuidado!",
	...
	public void playerBackToStart(int playerId, String type, int currentSquareId, int lastSquare) {
		...
		Timer delay = new Timer(2000, new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				getPlayersPanelCards().get(playerId - 1).getLblExpert()
						.setText(type + " - Vue. " + getPanelBoardGrid().getPlayerLap(currentSquareId) + "/2");
			}
		});
